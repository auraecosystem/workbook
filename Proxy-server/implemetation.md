...
## Advanced - Routing Strategies â­ï¸
...
**ð NEW** This is an async implementation of usage-based-routing.
...
router = Router(model_list=model_list, 
                ...
                routing_strategy="simple-shuffle", # ð RECOMMENDED - best performance
...
# init router
router = Router(model_list=model_list,
				routing_strategy="latency-based-routing",# ð set routing strategy
...
router.set_custom_routing_strategy(CustomRoutingStrategy()) # ð Set your routing strategy here
```
...
Behavior:
- `gpt-5.6-terra` â latency-based routing across the OpenAI + Azure deployments.
- `cheap-model` â simple-shuffle (the default group).
...
router_settings:
  ...
  routing_groups:
    - group_name: hot-path
      ...
      routing_strategy: latency-based-routing
      routing_strategy_args:
        ttl: 60          # short window â react quickly to latency changes
    - group_name: batch
...
model_list = [
    ...
    },
    {
        ...
        "litellm_params": {
            ...
            "order": 2,  # ð Tried when order=1 fails
        },
    },
]
...
model_list = [
	...
	},
	{
		...
		"litellm_params": {
			"model": "gpt-5.6-luna", 
			"api_key": os.getenv("OPENAI_API_KEY"), 
			"weight": 2 # ð PICK THIS DEPLOYMENT 2x MORE OFTEN THAN THE ONE ABOVE
		},
	},
]
...
router = Router(
    ...
    routing_strategy="simple-shuffle",
    enable_weighted_failover=True,  # ð retry within the same model group on failure
)
...
4. Steps 2â3 repeat until a deployment succeeds, every peer has been excluded, or `max_fallbacks` is reached.
...
model_list = [{
	...
	"litellm_params": {
		"model": "azure/gpt-5.6-terra",
		# ...
		"max_parallel_requests": 10 # ð SET PER DEPLOYMENT
	}
}]

### OR ### 

router = Router(model_list=model_list, default_max_parallel_requests=20) # ð SET DEFAULT MAX PARALLEL REQUESTS 
...
flowchart TD
    ...
    B --> C["Available:<br/>â¢ Anthropic Direct<br/>â¢ Vertex AI"]
...
from litellm import Router 
# ...
router = Router(model_list=model_list, enable_pre_call_checks=True) # ð Set to True
```
...
model_list = [
            {
                ...
                "litellm_params": {  # params for litellm completion/embedding call
                    ...
                    "api_version": os.getenv("AZURE_API_VERSION"),
                    "api_base": os.getenv("AZURE_API_BASE"),
					"region_name": "eu", # ð SET 'EU' REGION NAME
					"base_model": "azure/gpt-5.6-luna", # ð (Azure-only) SET BASE MODEL
                },
            },
            {
                "model_name": "gpt-5.6-luna", # model group name
            ...
            },
			{
				...
				"litellm_params": {
					...
					"vertex_location": "us-east1" # ð AUTOMATICALLY INFERS 'region_name'
				}
			}
        ]
...
**Solution** â :  Set `model_info["base_model"]` on your router init so litellm uses the correct model for calculating azure cost
